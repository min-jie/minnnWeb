...
function updateGraphMode(mode, hwNames = [], data = null) {
        ...
        try {
            ...
            Object.keys(results).forEach(hw => {
                converted[hw] = results[hw].map(item => {
                    ...
                    if (item && (item.Reviewer_Name || item.Author_Name || item.Assignment)) {
                        return item;
                    }
                    // 否則回傳 item 本體（降級處理）
                    return item;
    ...
    switch(mode) {
        case 'all': {
            console.log(`切換到All模式 (3個標籤score平均) (${targetHwNames.join(',')})`);
            if (typeof window.generateAllGraph === 'function') {
                ...
                console.warn('⚠️ 找不到 generateAllGraph，改用 generateAllLabelsGraph 作為後援');
        ...
        case 'relevance': {
            ...
            if (typeof window.generateRelevanceGraph === 'function') {
                ...
                console.warn('⚠️ 找不到 generateRelevanceGraph，改用 generateAllLabelsGraph 作為後援');
                window.generateAllLabelsGraph(useData, targetHwNames);
            } else {
        ...
        case 'concreteness': {
            ...
            if (typeof window.generateConcretenessGraph === 'function') {
                ...
                console.warn('⚠️ 找不到 generateConcretenessGraph，改用 generateAllLabelsGraph 作為後援');
                window.generateAllLabelsGraph(useData, targetHwNames);
            } else {
        ...
        case 'constructive': {
            ...
            if (typeof window.generateConstructiveGraph === 'function') {
                ...
                console.warn('⚠️ 找不到 generateConstructiveGraph，改用 generateAllLabelsGraph 作為後援');
                window.generateAllLabelsGraph(useData, targetHwNames);
            } else {
...
function prepareNetworkDataForBubbleChart(hwNames) {
    ...
    hwNames.forEach(hwName => {
        ...
        results[hwName] = hwData.map((assignment, idx) => {
            ...
            // 預設 predictions 欄位
            let relevance = 0, concreteness = 0, constructive = 0;
            let relevance_confidence = 0, concreteness_confidence = 0, constructive_confidence = 0;

            if (assignment.Round && Array.isArray(assignment.Round) && assignment.Round.length > 0) {
                let rCount = 0, cCount = 0, coCount = 0;
                let rSum = 0, cSum = 0, coSum = 0;
            ...
            return {
                ...
                predictions: {
                    ...
                    concreteness_confidence: Number((concreteness_confidence).toFixed(6)) || 0,
                    constructive: Number((constructive).toFixed(6)) || 0,
                    constructive_confidence: Number((constructive_confidence).toFixed(6)) || 0,
