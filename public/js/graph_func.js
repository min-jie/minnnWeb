...
function calculateReviewerLabelAverages(assignments) {
  ...
  assignments.forEach((assignment) => {
    ...
    if (reviewerStats[reviewer] && reviewerStats[reviewer].count > 0) {
      assignment.avgLabel =
        reviewerStats[reviewer].totalLabel / reviewerStats[reviewer].count;
    } else {
      assignment.avgLabel = NaN; // 若無有效數據，設為 NaN
    }
  });
...
function getAvgLabelQuality(rounds) {
  ...
  const validFeedbacks = rounds.filter(
  ...
  const totalLabel = validFeedbacks.reduce(
    (sum, r) => sum + (parseFloat(r.Label) || 0),
    0,
  );
  const avgLabelQuality = totalLabel / validFeedbacks.length;

  console.log(
    `📊 計算結果: totalLabel=${totalLabel}, avgLabelQuality=${avgLabelQuality}`,
  );
  ...
  hwNames.forEach((hwName) => {
    ...
    hwAssignments.forEach((assignment) => {
      ...
      rounds.forEach((round) => {
        ...
        // 調試信息：檢查數據值
        if (feedback !== "" && Math.random() < 0.1) {
          // 10% 機率顯示調試信息
          console.log(`[DEBUG] 審查者 ${reviewerId}:`);
          console.log(
            `  Relevance: ${relevance}, Concreteness: ${concreteness}, Constructive: ${constructive}`,
          );
      ...
      if (!hasNonEmptyFeedback && assignment && assignment.__predictions) {
        ...
        const WEIGHTS = { relevance: 30, concreteness: 30, constructive: 40 };
        const predScore =
          pRel * WEIGHTS.relevance +
          pCon * WEIGHTS.concreteness +
          pCst * WEIGHTS.constructive;
...
function generateAllLabelsGraph(rawData, hwName = ["HW1"]) {
  ...
  const sizeScale = (value) =>
    5 + ((value - minScore) / (maxScore - minScore)) * 25;
