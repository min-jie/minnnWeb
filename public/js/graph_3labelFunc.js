// 注意：這個文件依賴 graph_func.js 中的 processReviewerData 函數
// 確保 graph_func.js 在此文件之前載入

function updateNetworkInstance(container, data, options, rawData) {
  if (window.networkInstance) {
    ...
    window.networkInstance.on("click", function (properties) {
      if (properties.nodes.length > 0) {
        ...
        const selectedHWs = Array.from(
          document.getElementById("hw-select").selectedOptions,
        ).map((opt) => opt.value);
        const reviewerRecords = selectedHWs.flatMap(
          (hwName) =>
...
function generateGraph(rawData, mode, hwNames) {
  ...
  // 資料處理 - 使用全域函數
  const { nodes, links } = window.processReviewerData(rawData, mode, hwNames);
  ...
  const visNodes = nodes.map((n) => {
    ...
    const completionRate =
      assignmentCount > 0 ? completedAssignments / assignmentCount : 0;
    ...
    if (mode === "all") {
      ...
      if (totalFeedbacks > 0) {
        ...
        if (n.id === "D1018525" || Math.random() < 0.05) {
          ...
          console.log(
            `  相關性標籤: ${n.labelCounts.relevance} (score: ${relevanceScore.toFixed(3)})`,
          );
          console.log(
            `  具體性標籤: ${n.labelCounts.concreteness} (score: ${concretenessScore.toFixed(3)})`,
          );
          console.log(
            `  建設性標籤: ${n.labelCounts.constructive} (score: ${constructiveScore.toFixed(3)})`,
          );
      ...
      // 單一標籤模式的調試信息
      if (Math.random() < 0.1) {
        console.log(`[${mode.toUpperCase()} Mode] 審查者 ${n.id}:`);
        console.log(`  有效評論Round數: ${totalFeedbacks}`);
        console.log(`  ${mode}標籤數: ${n.labelCounts[mode]}`);
        console.log(`  ${mode} score: ${score.toFixed(3)}`);
        console.log(`  labelCounts:`, n.labelCounts);
      }
    }
    ...
    let color;
    if (score >= 0.75)
      color = colorConfig[mode].colors[3]; // 最深色 (75%以上)
    else if (score >= 0.5)
      color = colorConfig[mode].colors[2]; // 深色 (50-75%)
    else if (score >= 0.25)
      color = colorConfig[mode].colors[1]; // 淺色 (25-50%)
    else color = colorConfig[mode].colors[0]; // 最淺色 (25%以下)

    // 調試信息：顏色分級
    if (Math.random() < 0.15) {
      // 15% 機率顯示調試信息
      const tier = score >= 0.75 ? 4 : score >= 0.5 ? 3 : score >= 0.25 ? 2 : 1;
      console.log(
        `[COLOR DEBUG] 審查者 ${n.id}: score=${score.toFixed(3)}, color=${color}, tier=${tier}, mode=${mode}`,
      );
    }
  ...
  const visEdges = links.map((e) => ({
    ...
    to: e.to,
    color: {
      color: e.completedAll ? "#73BEFF" : "#ff6b6b",
...
function generateRelevanceGraph(rawData, hwNames = ["HW1"]) {
  generateGraph(rawData, "relevance", hwNames);
}

function generateConcretenessGraph(rawData, hwNames = ["HW1"]) {
  generateGraph(rawData, "concreteness", hwNames);
}

function generateConstructiveGraph(rawData, hwNames = ["HW1"]) {
  generateGraph(rawData, "constructive", hwNames);
}

function generateAllGraph(rawData, hwNames = ["HW1"]) {
  generateGraph(rawData, "all", hwNames);
}

// 將函數暴露到全域
window.updateNetworkInstance = updateNetworkInstance;
