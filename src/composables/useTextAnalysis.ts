...
export function useTextAnalysis() {
  ...
  const allResults = computed((): AnalysisItem[] => {
    ...
    if (
      analysisResults.value.detailed_results &&
      Array.isArray(analysisResults.value.detailed_results)
    ) {
  ...
  const statisticsData = computed(() => {
    ...
    const totalPositive =
      stats.relevance + stats.concreteness + stats.constructive;
  ...
  const performAnalysis = async (): Promise<boolean> => {
    ...
    try {
      ...
      // 直接上傳檔案，不在前端解析
      const result = await textAnalysisAPI.uploadFileForAnalysis(
        selectedFile.value,
        [
